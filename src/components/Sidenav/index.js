import React from 'react';
import './style.css';

function Sidenav() {
  return (
    <>
      <nav className="navbar">
        {/* Avatar */}
        <div className="flex-container">
          <div className="row">
            <a href="/" className="logo">
              <img id="avatar" src="./assets/avatar-hoodie.svg" alt="Cheng avatar"></img>
            </a>
            <h3 className="logo">hi. my name is Cheng.</h3>
          </div>
        </div>

        {/* Page links */}
        <div className="row">
          <div className="col">
            <ul className="navbar-nav">
              <li className="nav-item">
                <a href="#about" className="nav-link">
                  {/* <img className="icons" src="./assets/info.svg" alt="info icon"></img> */}
                  {/* <svg
                  className="icons"
                  width="70"
                  height="70"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M11 10.9794C11 10.4271 11.4477 9.97937 12 9.97937C12.5523 9.97937 13 10.4271 13 10.9794V16.9794C13 17.5317 12.5523 17.9794 12 17.9794C11.4477 17.9794 11 17.5317 11 16.9794V10.9794Z"
                    fill="currentColor"
                  />
                  <path
                    d="M12 6.05115C11.4477 6.05115 11 6.49886 11 7.05115C11 7.60343 11.4477 8.05115 12 8.05115C12.5523 8.05115 13 7.60343 13 7.05115C13 6.49886 12.5523 6.05115 12 6.05115Z"
                    fill="currentColor"
                  />
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22C17.5228 22 22 17.5228 22 12C22 6.47715 17.5228 2 12 2ZM4 12C4 16.4183 7.58172 20 12 20C16.4183 20 20 16.4183 20 12C20 7.58172 16.4183 4 12 4C7.58172 4 4 7.58172 4 12Z"
                    fill="currentColor"
                  />
                </svg> */}
                  <span className="link-text">about</span>
                </a>
              </li>

              <li className="nav-item">
                <a href="#work" className="nav-link">
                  {/* <img className="icons" src="./assets/code-slash.svg" alt="code icon"></img> */}
                  <span className="link-text">work</span>
                </a>
              </li>

              <li className="nav-item">
                <a href="#experience" className="nav-link">
                  {/* <img className="icons" src="./assets/work-alt.svg" alt="briefcase icon"></img> */}
                  <span className="link-text">experience</span>
                </a>
              </li>

              <li className="nav-item">
                <a href="#contact" className="nav-link">
                  {/* <img className="icons" src="./assets/phone.svg" alt="phone icon"></img> */}
                  <span className="link-text">contact</span>
                </a>
              </li>
            </ul>
          </div>
        </div>

        {/* Social links */}
        {/* <div className="flex-container">
          <div className="row">
            <div className="col">
              <ul className="navbar-social">
                <li className="social-item">
                  <a href="https://cheng21tang.medium.com/" className="social-link">
                    <img id="medium" src="./assets/Artboard 1.svg" alt="medium logo"></img>
                  </a>
                </li>
              </ul>
            </div>
          </div>
        </div> */}
      </nav>
    </>
  )
}

export default Sidenav;